...
def register_all_styles() -> None:
    ...
    -----
    RuntimeWarning
        If an overlay key collides with a journal spec key under the same
        ``"plotstyle."`` prefix.  The overlay takes precedence in that case.
    """
    import contextlib
    import warnings
    ...
    registered_journal_keys: set[str] = set()
    ...
    for key in overlay_registry.list_available():
        full_key = f"{_PREFIX}{key}"
        if full_key in registered_journal_keys:
            from plotstyle._utils.warnings import PlotStyleWarning

            warnings.warn(
                f"plotstyle: overlay {key!r} collides with a journal spec of the same name; "
                "the overlay will shadow the journal preset.",
                PlotStyleWarning,
                stacklevel=2,
            )
        with contextlib.suppress(Exception):
            library[full_key] = build_overlay_snapshot(overlay_registry.get(key))
